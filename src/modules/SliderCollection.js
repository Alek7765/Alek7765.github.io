...
class Slider {
  ...
  update = () => {
    const isTablet = MatchMedia.tablet.matches
  ...
  bindEvents() {
    MatchMedia.tablet.addEventListener('change', this.update)
  }
}
...
class SliderCollection {
  ...
  init() {
    document.querySelectorAll(rootSelector).forEach((element) => {
      new Slider(element) // инициализируем слайдер с переданным элементом из rootSelector
    })
  }
}
